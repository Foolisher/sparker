...
object Demo1 {
  ...
  def main(args: Array[String]) {
    ...
    CassandraConnector(conf).withSessionDo(session=>{
      ...
      if (session.execute("select count(*) from groupon.ecp_orders;").one().getLong("count") < 0) {
    ...
    print("卖家贡献: ")
    rdd.filter(row=>row.getInt("has_paid")==1)
      .map(row=>(row.getString("buyer_name"), (row.getInt("fee")/100.00).toFloat))
      .reduceByKey(_ + _)
      .collect()
